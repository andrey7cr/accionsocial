...
class GmapDefaults {
  ...
  private function __construct() {
    ...
    $gmap_marker_file = file_load(variable_get('gmap_marker_file', false));
    if($gmap_marker_file) {
      $this->basejs[file_create_url($gmap_marker_file->uri)] = array(
        'type' => 'external',
        'weight' => 4,
      );
    }
    else {
      watchdog('gmap', 'A valid marker cache file could not be found, please regenerate it <a href="@url">here</a>', array('@url' => '/admin/config/services/gmap'), WATCHDOG_ERROR);
    }
  }
